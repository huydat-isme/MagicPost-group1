...
export default function SearchPackage() {
  ...
  const [gatheringPointSearchQuery, setGatheringPointSearchQuery] =
    useState("");
  ...
  const handleOrderSearchButtonClick = () => {
    ...
    if (searchValue) {
      ...
      console.error(
        "Giá trị tìm kiếm không hợp lệ. Hãy nhập một giá trị hợp lệ."
      );
      setFilteredOrders([]);
    }
  };
  ...
  const handleGatheringPointSearchButtonClick = () => {
    ...
    if (searchValue) {
      ...
      fetch(`http://localhost:3000/api/gatheringpoint/${searchValue}`, {
        ...
        .then((gatheringData) => {
          if (
            gatheringData.status === "success" &&
            gatheringData.data &&
            gatheringData.data.gatheringPoints
          ) {
        ...
        .catch((error) =>
          console.error("Error fetching gathering point data:", error)
        );
    } else {
      console.error(
        "Giá trị tìm kiếm không hợp lệ. Hãy nhập một giá trị hợp lệ."
              ...
              <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                  {gatheringPoints.map((gatheringPoint) => (
                    <tr
                      key={gatheringPoint.id}
                      className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700"
                    >
                      {/* Example columns, adjust as needed */}
                      <td className="px-6 py-4">{gatheringPoint.id}</td>
                      <td className="px-6 py-4">
                        {gatheringPoint.gathering_location.city}
                      </td>
                      <td className="px-6 py-4">
                        {gatheringPoint.gathering_user.username}
                      </td>
                      <td className="px-6 py-4">
                        {gatheringPoint.gathering_order.code}
                      </td>
                      {/* ... (other columns) */}
                    </tr>
                  ))}
